...
- Repo: `git init -b main`, committed, and pushed to
  ...
  authenticated as `louisweitai0903`. All changes through the domain fix
  below are pushed to `main`.
- Deployed on Vercel by the user at
  `https://mbs-website-ten.vercel.app/`, connected to `main` (auto-deploys
  on push — no separate deploy step needed going forward, just push).
- `index.html`: added favicon `<link>`s, OG/Twitter meta tags (image URLs
  now point at the real live domain), scroll-progress markup
  (`#scroll-progress`, `#scroll-progress-bar`, `#scroll-progress-pct`),
  and swapped both `assets/mark.png` references to `assets/mercedes.png`.
...
- Site is live and deployed; nothing blocking. Pushing to `main` redeploys
  automatically via the Vercel/GitHub integration.
- Consider asking the client if a real MBS logo/mark exists to replace the
  Mercedes-Benz badge stand-in (see Failed attempts above for why
  `mark.png` couldn't just be re-fetched).
- If a custom domain is ever added, update it in the same three spots as
  before: `robots.txt`, `sitemap.xml`, `index.html`'s OG/Twitter tags.
